...
class BaseAgent:
    ...
    def __init__(self, config):
        ...
        if "dino" in config['run_name'].lower():
            ...
            self.data_loaders: Dict[str, DataLoader] = dict(zip(
                ["train", "val", "test"], 
                create_echoprime_dataloaders(config)  # Returns (train, val, test)
            ))        
        else:    
            self.data_loaders: Dict[str, DataLoader] = dict(zip(
                ["train", "val", "test"], 
                create_video_dataloaders(config)  # Returns (train, val, test)
            ))

        self.logit_names = ["Normal", "Elevated"]
    ...
    def create_pred_log_df(self, data_sample, logit, logit_names):
        ...
        """
        # pred_log_data = {
        #     "filename": data_sample["stream_id"],
        #     "exam_id": data_sample["exam_id"],
        #     "label": data_sample["label"].int().numpy(),
        # }
        ...
        pred_log_data = {
            "filename": data_sample["stream_id"],
            "exam_id": data_sample["exam_id"], 
            "label": data_sample["label"].int().cpu().numpy().flatten(),  # ← Add .flatten()
        }
        
        # Fix logits: squeeze/transpose → 1D per class
        logit_t = logit.t().cpu().numpy()  # (C, B) → CPU numpy
        for as_label, value in zip(logit_names, logit_t):
            pred_log_data[f"logit_{as_label}"] = value.flatten()  # ← 1D array!
